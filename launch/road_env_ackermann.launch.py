...
def create_spawn_actions(context, *args, **kwargs):
    ...
    spawn_controller = Node(
        ...
        executable="spawner",
        output="screen",
        arguments=[controller_name, "--controller-manager", cm_fqn],
    )
...
def generate_launch_description():
    ...
    controller_name = LaunchConfiguration("ackermann_controller")
    ...
    # Robot state publisher launch (with robot_type=ackermann)
    ...
    # Actions to run after course generation
    delayed_gazebo_launch = RegisterEventHandler(
        OnProcessExit(
            target_action=generate_course_cmd,
            on_exit=[
                LogInfo(msg="Course generation completed. Starting Gazebo with Ackermann robot..."),
                gazebo_launch,
                rsp_launch,
    ...
    # --- Actions for when generate_course is false ---
    ...
    spawn_controller_immediate = Node(
        package="controller_manager",
        executable="spawner",
        output="screen",
        arguments=[controller_name, "--controller-manager", cm_fqn],
    ...
    immediate_controllers = TimerAction(
        period=6.0,
        actions=[spawn_jsb_immediate, spawn_controller_immediate],
        condition=UnlessCondition(generate_course),
    )
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("ackermann_controller", default_value="ackermann_steering_controller"),
        DeclareLaunchArgument(
            "seed",
            default_value="",
